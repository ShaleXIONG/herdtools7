...
(* Prefix *)
    ...
    let call_rec prefix f0 safes po_safe over n r suff f_rec k ?(reject=[])=
        if O.verbose > 2 then eprintf "Call_rec with over: %b, suff: %s \n" over (pp_ess suff) ;
      if
      ...
      then
        ...
        if n <= 0 then k
            ...
            eprintf "f_rec call [in call_rec] with %s \n" (pp_ess suff) ;
            f_rec n suff k
        end
      else k
...
(* Add safe edge to suffix *)
        let rec add_safe over ss n suf k =
          List.fold_left (fun k s ->
            eprintf "add_safe [in choose_relax]: %s\n" (pp_ess suf) ;
            call_rec over n s suf (add_relaxs over) k
          ) k ss
(* Addome relax edges `relex_edge` to suffix, or nothing *)
        and add_relaxs over n suf k =
            eprintf "add_relaxs [in choose_relax]: %s\n" (pp_ess suf) ;
          let k = call_rec true n relex_edge suf (add_relaxs true) k in
          add_safe over safe n suf k in

        if List.is_empty prefix then begin
          (* Optimise: start with a relax edge `relex_edge` *)
            eprintf "prefix [in choose_relax] is empty\n" ;
            let k = call_rec true n relex_edge [] (add_relaxs true) k in
            k
        end else begin
            eprintf "prefix [in choose_relax] is NOT empty\n" ;
...
(* Is xs a prefix of s@p ? *)
    ...
    let last_check_call rej aset f rs po_safe res k =
      if List.is_empty res then k else
    ...
    let zyva_prefix prefixes aset relax safe reject n f k =
      List.fold_left ( fun k pref ->
         zyva pref aset relax safe reject n f k
      ) k prefixes
