...
let emit_joker st init = None,init,[],st
    ...
    let emit_exch st p init er ew =
      ...
      let rR,st = A.alloc_reg st in
      let rW,init,csv,st = U.emit_mov st p init (Value.value_to_int ew.v) in
      let cs,st = emit_pair p st rR rW rA in
      rR,init,csv@cs,st
      ...
      | Some W,Data loc ->
          ...
          let cs2 =
            [Instruction (OP (XOR,r2,r1,r1)) ;
             Instruction (OPI (ADDU,r2,r2,(Value.value_to_int e.v))) ; ] in
...
(* Check load *)
    let do_check_load p st r e =
      let ok,st = A.ok_reg st in
      (fun k -> lift_code (branch_neq r (Value.value_to_int e.v) (Label.last p) [inc ok])@k),
      A.next_ok st
