...
*)
  ...
  let make_wsi idx loc = { evt_null with dir=Some W ; loc=loc; idx=idx; v=Value.value_of_int 0;}
  ...
  let debug_vector =
    if do_neon || do_sve || do_sme then
      let pp_one value = Code.add_vector O.hexa 
        (List.map Value.value_to_int value) in
      fun e ->
      sprintf " (vecreg={%s})"
...
module CoSt = struct
  ...
  type t = { map : int M.t; co_cell : Value.v array; }
  ...
  let create ?(init=0) sz =
    ...
    and co_cell = Array.make (if sz <= 0 then 1 else sz) (Value.value_of_int init) in
    { map; co_cell;  }

  let find_no_fail key map =
    try M.find key map with Not_found -> assert false

  let get_co st bank = Value.value_of_int (find_no_fail bank st.map)
    ...
    | Ord|Pair -> begin
       ...
       begin
         ...
         | Pair -> (* No Rmw for pairs *)
            ...
            let width = Value.value_of_int ((Value.value_to_int e.v) - 1) in
  ...
  let step_simd st n =
    ...
    let new_co_cell = st.co_cell |> Array.map Value.value_to_int 
                      |> E.SIMD.step n fst 
                      |> Array.map Value.value_of_int in
    { co_cell=new_co_cell;
    ...
    | n::ns ->
       ...
       begin if Code.is_data n.evt.loc then
          begin if do_memtag then
            let tag = Value.value_to_int (CoSt.get_co st Tag) in
            n.evt <- { n.evt with tag=tag; }
          else if do_morello then
            let ord = Value.value_to_int (CoSt.get_co st Ord) in
            let ctag = Value.value_to_int (CoSt.get_co st CapaTag) in
            let cseal = Value.value_to_int (CoSt.get_co st CapaSeal) in
            ...
            | Data _ ->
                ...
                | VecReg a ->
                   ...
                   let cell = CoSt.get_cell st
                                |> Array.map Value.value_to_int in
                   let vecreg  = E.SIMD.read a cell
                            |> List.map (List.map Value.value_of_int) in
                   let cell = Array.map Value.value_of_int cell in
                   let v =
                     match vecreg with
                       | (v::_)::_ -> v
                       | _ -> assert false in
      ...
      List.fold_right
        (fun ns (k,env as r) ->
          ...
          | n::_ ->
              ...
              let env = if do_kvm then (Code.as_data loc,Value.value_of_int k)::env else env in
              if next_x_ok then
                k+8,(next_x,Value.value_of_int (k+4))::env
              else
                k+4,env)
        nss (0,[]) in
    initvals
...
let set_dep_v nss =
  let v = List.fold_left
    (fun k ns ->
      List.fold_left
        (fun v n ->
          n.evt <- { n.evt with dep=Value.value_to_int v; } ;
          n.evt.v)
        k ns)
    (Value.value_of_int 0) nss in
...
let set_read_pair_v n cell =
  let e = n.evt in
  let v0 = E.extract_value cell.(0) e.atom |> Value.value_to_int
  and v1 =  E.extract_value cell.(1) e.atom |> Value.value_to_int in
  let v = v0 + v1 |> Value.value_of_int in
  let e = { e with v=v; } in
  n.evt <- e
  ...
  let rec do_rec st cell pte_cell = function
    ...
    | n::ns ->
        ...
        | Some R ->
            ...
            | VecReg a ->
               let cell = Array.map Value.value_to_int cell in
               let v = E.SIMD.read a cell 
                        |> E.SIMD.reduce 
                        |> Value.value_of_int in
        ...
        | Some W ->
            let st =
              match bank with
              | Tag|CapaTag|CapaSeal ->
                 CoSt.set_co st bank (Value.value_to_int n.evt.v)
...
(* Set write values *)
  ...
  if O.verbose > 1 then begin
    eprintf "INITIAL VALUES: %s\n"
      (String.concat "; "
         (List.map
            (fun (loc,k) -> sprintf "%s->%d" loc (Value.value_to_int k))
            initvals)) ;
      ...
      (String.concat ","
         (List.map
            (fun (loc,(v,_pte)) -> sprintf "%s -> 0x%x"
                (Code.pp_loc loc) (Value.value_to_int v)) vs))
  end ;
