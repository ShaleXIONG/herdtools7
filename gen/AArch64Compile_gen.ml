...
(**********)
    ...
    module STG = struct
      ...
      let emit_store st p init e =
        ...
        and v = Code.add_tag loc (Value.value_to_int e.C.v) in
...
(********)
    ...
    let emit_stp_reg opt idx st p init rA e =
      match opt,idx with
      | _,Both ->
      do_emit_stp_reg opt st p init (Value.value_to_int e.C.v) rA
...
(**********)
    ...
    let do_emit_exch1 emit_addr st p init er ew =
      ...
      let rR,st = next_reg st in
      let rW,init,csi,st = U.emit_mov st p init (Value.value_to_int ew.C.v) in
    ...
    let do_emit_exch22 emit_addr st p init er ew =
      ...
      let rW1,init,csi,st = U.emit_mov st p init (Value.value_to_int ew.C.v) in
    ...
    let do_emit_ldop_rA  ins ins_mixed st p init er ew rA =
      ...
      let rR,st = next_reg st in
      let rW,init,csi,st = mk_emit_mov sz st p init (Value.value_to_int ew.C.v) in
    ...
    let emit_cas_rA st p init er ew rA =
      assert (er.C.ctag = ew.C.ctag && er.C.cseal = ew.C.cseal) ;
      let sz,a,opt = do_rmw_annot (tr_none er.C.atom) (tr_none ew.C.atom) in
      let rS,init,csS,st = mk_emit_mov_fresh sz st p init (Value.value_to_int er.C.v) in
      let rT,init,csT,st = mk_emit_mov sz st p init (Value.value_to_int ew.C.v) in
      let sz = match opt with
      | None -> sz
      | Some Capability -> assert (Misc.is_none sz) ; Some (MachSize.S128, 0) in
...
(* Dependencies *)
    ...
    let do_check_load p st r e =
      let ok,st = A.ok_reg st in
      (fun k ->
        Instruction (cmpi r (Value.value_to_int e.C.v))::
